...
const createCollege = async (req, res) => {
    const { cityId, collegeName, collegeAddress } = req.body;
    const clg = await college.findOne({ collegeName });
    if(clg)
    {
        return res.status(400).json({ message: "College already exists" });
    }
    const newCollege = new college({ cityId, collegeName, collegeAddress });
    try {
...
const updateCollege = async (req, res) => 
{
    ...
    try {
        await college.updateOne({_id: clgid}, { cityId, collegeName, collegeAddress });
